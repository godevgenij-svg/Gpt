...
function Configure-QB {
    ...
    # qBittorrent uses QSettings INI keys with literal single backslashes.
@"
[LegalNotice]
Accepted=true
...
function Configure-Amule($S){
    ...
    # Current aMule explicitly persists this flag and uses it to suppress the fresh-install GUI wizard.
...
function Ensure-Prowlarr-Key {
    $key=Prowlarr-Key
    if($key){return $key}
    # ApiKey is generated by ConfigFileProvider on first property access and persisted.
    # An expected unauthenticated API request forces AuthenticationHandler to access it.
    try { Invoke-WebRequest -UseBasicParsing 'http://127.0.0.1:9696/api/v1/indexer' -TimeoutSec 5 | Out-Null } catch {}
    for($n=0;$n-lt30;$n++){Start-Sleep -Milliseconds 250;$key=Prowlarr-Key;if($key){return $key}}
    throw 'Prowlarr did not generate ApiKey'
}
...
function Start-All {
    ...
    # Use the normal Windows client core after suppressing the first-run wizard; its OnInit launches amuleapi with ephemeral EC credentials.
    $am=Find-Exe amule.exe (Join-Path $Backends aMule);[void](Start-One $am @("--config-dir=$(Join-Path $Data aMule)") -Hidden)
