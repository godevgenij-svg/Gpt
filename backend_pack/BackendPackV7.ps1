...
function Ensure-Secrets {
    ...
    $o=[ordered]@{
        ...
        qBittorrentWebPassword=New-Secret 20
...
function New-QBPasswordHash([string]$Password){
    $salt=New-Object byte[] 16;$rng=[Security.Cryptography.RandomNumberGenerator]::Create();try{$rng.GetBytes($salt)}finally{$rng.Dispose()}
    $kdf=[Security.Cryptography.Rfc2898DeriveBytes]::new($Password,$salt,100000,[Security.Cryptography.HashAlgorithmName]::SHA512)
...
function Set-Ini([string]$Path,[string]$Section,[string]$Key,[string]$Value){
    $lines=@();if(Test-Path $Path){$lines=@(Get-Content $Path)};$head="[$Section]";$s=-1;$e=$lines.Count
...
function Configure-QB($S){
    ...
    $save=(Join-Path $Downloads BitTorrent).Replace('\','/')+'/';$hash=New-QBPasswordHash ([string]$S.qBittorrentWebPassword)
@"
[LegalNotice]
Accepted=true
...
WebUI\UseUPnP=false
"@|ForEach-Object { $_ -replace '\\\\','\' }|Set-Content (Join-Path $dir qBittorrent.ini) -Encoding UTF8
}

function Configure-Amule($S){
    $dir=Join-Path $Data aMule;Ensure-Dir $dir;$conf=Join-Path $dir amule.conf
    if(-not(Test-Path $conf)){@('[eMule]','FirstRunWizardDone=1')|Set-Content $conf -Encoding UTF8}
    $api=Find-Exe amuleapi.exe (Join-Path $Backends aMule)
    Set-Ini $conf eMule FirstRunWizardDone 1
    Set-Ini $conf eMule ConnectToKad 1
    Set-Ini $conf eMule ConnectToED2K 1
...
function Write-BLConfig($S,[array]$Sources=@()){
    Ensure-Dir $Prepared;$out=Join-Path $Prepared ExternalSearch.xml;$sl=XE ([string]$S.slskdApiKey);$am=XE ([string]$S.amuleApiPassword);$qb=XE ([string]$S.qBittorrentWebPassword);$bt=XE (Join-Path $Downloads BitTorrent);$src=''
...
<ExternalSearch Version="4">
  ...
  <QBittorrent Enabled="1" BaseUrl="http://127.0.0.1:8080" ApiKey="" Username="blacklink" Password="$qb" SavePath="$bt" Category="" />
  <Torznab>
$src  </Torznab>
</ExternalSearch>
...
function Start-All {
    ...
    foreach($x in @(@('slskd',5030),@('Prowlarr',9696),@('qBittorrent',8080),@('amuleapi',4713))){if(Wait-Port $x[1] 60){Write-Host("{0,-12} OK 127.0.0.1:{1}"-f$x[0],$x[1])}else{Write-Warning "$($x[0]) did not open port $($x[1])"}}
}
...
function Sync-Prowlarr {
    ...
    $key=[string]$s.prowlarrApiKey;$uri='http://127.0.0.1:9696/api/v1/indexer?apikey='+[uri]::EscapeDataString($key);$items=$null;$err=$null
...
function Setup {foreach($d in @($Backends,$Data,$Prepared,$Downloads)){Ensure-Dir $d};$s=Ensure-Secrets;Configure-Slskd $s;Configure-Prowlarr $s;Configure-QB $s;Configure-Amule $s;$x=Write-BLConfig $s;Install-BL $x;Start-All;Sync-Prowlarr;if(-not$NonInteractive){Write-Host 'Prowlarr: http://127.0.0.1:9696';Write-Host 'Add wanted torrent indexers, then run SYNC_PROWLARR.cmd.';try{Start-Process 'http://127.0.0.1:9696'|Out-Null}catch{}}}
