...
$tmp=Join-Path $env:TEMP ('bl-backends-v7-'+[guid]::NewGuid().ToString('N'));$pack=Join-Path $tmp BlackLink_Backends_Ready_x64;New-Item -ItemType Directory -Path $tmp,$pack,(Join-Path $pack Backends) -Force|Out-Null
...
try{
 ...
 $q=Join-Path $tmp qb.exe;DL 'https://github.com/qbittorrent/qBittorrent/releases/download/release-5.2.3/qbittorrent_5.2.3_x64_setup.exe' $q 'ff508e2f912d59c9eabaf03633ebacfd45c2049f38dcac027b8a7d7ad867ab2f';$qd=Join-Path $pack 'Backends\qBittorrent';New-Item -ItemType Directory $qd -Force|Out-Null;$qp=Start-Process $q -ArgumentList @('/S',"/D=$qd") -Wait -PassThru;if($qp.ExitCode-ne0){throw "qB install $($qp.ExitCode)"}
 $az=Join-Path $tmp amule.zip;& curl.exe -fL --retry 4 --retry-delay 2 'https://nightly.link/amule-org/amule/actions/artifacts/9246296823.zip' -o $az;if($LASTEXITCODE){throw 'aMule artifact download failed'};$ao=Join-Path $tmp amuleouter;Expand-Archive $az $ao -Force;$inner=Get-ChildItem $ao -Recurse -File -Filter '*Windows-x64*.zip'|Select-Object -First 1;if(-not$inner){throw 'aMule package missing'};$ih=(Get-FileHash $inner.FullName -Algorithm SHA256).Hash.ToLowerInvariant();if($ih-ne'a6d6bb99e064b67951608760bd4e095720847abb1402c135c0b79a5bf68fe559'){throw "aMule hash mismatch $ih"};Expand-Archive $inner.FullName (Join-Path $pack 'Backends\aMule') -Force
 foreach($e in 'slskd.exe','Prowlarr.exe','qbittorrent.exe','amuled.exe','amuleapi.exe'){if(-not(Get-ChildItem (Join-Path $pack Backends) -Recurse -File -Filter $e|Select-Object -First 1)){throw "$e missing"}}

 # Generate the shipped runtime script from the reviewed V6 source with four narrowly-scoped fixes
 # proven by source inspection and the dedicated Windows runtime diagnostic.
 $rt=Get-Content (Join-Path $PSScriptRoot BackendPackV6.ps1) -Raw
 $old="$o=[ordered]@{slskdApiKey=New-Secret 32;slskdWebPassword=New-Secret 20;amuleApiPassword=New-Secret 20}"
 $new="$o=[ordered]@{slskdApiKey=New-Secret 32;slskdWebPassword=New-Secret 20;amuleApiPassword=New-Secret 20;prowlarrApiKey=New-Secret 16}"
 $rt=$rt.Replace($old,$new)
 $rt=$rt.Replace('function Configure-Prowlarr {','function Configure-Prowlarr($S) {')
 $rt=$rt.Replace('  <LaunchBrowser>False</LaunchBrowser>',"  <LaunchBrowser>False</LaunchBrowser>`r`n  <ApiKey>$(XE ([string]`$S.prowlarrApiKey))</ApiKey>")
 $rt=$rt.Replace('WebUI\\LocalHostAuth=false',"WebUI\\LocalHostAuth=false`r`nWebUI\\Username=blacklink`r`nWebUI\\Password_PBKDF2=`"@ByteArray(+nqIve0kGenX6Anl3N+SPA==:Lyx2vXZFlnU2k+27EvmMHddocLMRcNuu4+1/T6gojxcekfxUH0wxr51rKwKc/KpdNMGZe/4WpH0dtKBi9LzhaQ==)`"")
 $rt=$rt.Replace('Set-Ini $conf eMule FirstRunWizardDone 1',"Set-Ini `$conf eMule FirstRunWizardDone 1`r`n    Set-Ini `$conf ExternalConnect AcceptExternalConnections 1`r`n    Set-Ini `$conf ExternalConnect ECAddress 127.0.0.1`r`n    Set-Ini `$conf ExternalConnect ECPort 4712")
 $rt=$rt.Replace("`$am=Find-Exe amule.exe (Join-Path `$Backends aMule)","`$am=Find-Exe amuled.exe (Join-Path `$Backends aMule)")
 ...
 if($rt -notmatch 'prowlarrApiKey=New-Secret' -or $rt -notmatch 'AcceptExternalConnections 1' -or $rt -notmatch 'Password_PBKDF2' -or $rt -notmatch 'Find-Exe amuled.exe'){throw 'Runtime patch generation failed'}
...
@echo off
cd /d "%~dp0"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0BackendPack.ps1" -Action $($pair[1])
pause
...
aMule official Actions artifact 9246296823 commit 68eb98885dfcdaed407c9b0ace4dacd5fb8065ea inner SHA256 a6d6bb99e064b67951608760bd4e095720847abb1402c135c0b79a5bf68fe559
"@|Set-Content (Join-Path $pack VERSIONS.txt) -Encoding UTF8

 # Full Windows smoke in a disposable copy.
 $sm=Join-Path $tmp smoke;Copy-Item $pack $sm -Recurse -Force;$r=Join-Path $sm BackendPack.ps1;& $r -Action setup -NonInteractive -SoulseekUsername blacklink_ci_probe -SoulseekPassword blacklink_ci_probe
 ...
 foreach($p in 5030,9696,8080,4713){if(-not(WP $p 20)){& $r -Action status;Get-ChildItem (Join-Path $sm Data) -Recurse -File -ErrorAction SilentlyContinue|Where-Object{$_.Name-match'log|conf|ini|xml|yml'}|ForEach-Object{Write-Host "--- $($_.FullName)";Get-Content $_.FullName -Tail 160 -ErrorAction SilentlyContinue};throw "Port $p failed"}}
 ...
 & $r -Action sync
 [xml]$px=Get-Content (Join-Path $sm 'Data\Prowlarr\config.xml') -Raw;$pk=[string]$px.Config.ApiKey;if(-not$pk){throw 'Prowlarr API key missing'};$pr=Invoke-WebRequest -UseBasicParsing ('http://127.0.0.1:9696/api/v1/indexer?apikey='+[uri]::EscapeDataString($pk)) -TimeoutSec 10;if($pr.StatusCode-ne200){throw 'Prowlarr API failed'}
...
}finally{try{if(Test-Path (Join-Path $tmp 'smoke\BackendPack.ps1')){& (Join-Path $tmp 'smoke\BackendPack.ps1') -Action stop}}catch{};Remove-Item $tmp -Recurse -Force -ErrorAction SilentlyContinue}
