...
$tmp=Join-Path $env:TEMP ('bl-backends-'+[guid]::NewGuid().ToString('N'));$pack=Join-Path $tmp BlackLink_Backends_Ready_x64;New-Item -ItemType Directory -Path $tmp,$pack,(Join-Path $pack Backends) -Force|Out-Null
function DL($u,$o,$sha){& curl.exe -fL --retry 4 --retry-delay 2 $u -o $o;if($LASTEXITCODE){throw "Download failed $u"};$h=(Get-FileHash $o -Algorithm SHA256).Hash.ToLowerInvariant();if($h-ne$sha){throw "SHA mismatch $o : $h"}}
try{
 ...
 $q=Join-Path $tmp qb.exe;DL 'https://github.com/qbittorrent/qBittorrent/releases/download/release-5.2.3/qbittorrent_5.2.3_x64_setup.exe' $q 'ff508e2f912d59c9eabaf03633ebacfd45c2049f38dcac027b8a7d7ad867ab2f';$qe=Join-Path $tmp qb;New-Item -ItemType Directory $qe -Force|Out-Null;& 7z.exe x $q "-o$qe" -y|Out-Null;if($LASTEXITCODE){throw 'qBittorrent extraction failed'};$qx=Get-ChildItem $qe -Recurse -File -Filter qbittorrent.exe|Select-Object -First 1;if(-not$qx){throw 'qbittorrent.exe missing'};$qd=Join-Path $pack 'Backends\qBittorrent';New-Item -ItemType Directory $qd -Force|Out-Null;Copy-Item (Join-Path $qx.Directory.FullName '*') $qd -Recurse -Force
 ...
 Copy-Item (Join-Path $PSScriptRoot BackendPackV5.ps1) $pack -Force
 foreach($pair in @(@('SETUP_AND_START.cmd','setup'),@('START_ALL.cmd','start'),@('STOP_ALL.cmd','stop'),@('SYNC_PROWLARR.cmd','sync'),@('STATUS.cmd','status'))){@"
@echo off
cd /d "%~dp0"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0BackendPackV5.ps1" -Action $($pair[1])
pause
...
"@|Set-Content (Join-Path $pack VERSIONS.txt) -Encoding UTF8
 ...
 # Smoke in disposable copy. Network credentials are intentionally fake; only local API lifecycle is tested.
 $sm=Join-Path $tmp smoke;Copy-Item $pack $sm -Recurse -Force;$r=Join-Path $sm BackendPackV5.ps1;& $r -Action setup -NonInteractive -SoulseekUsername blacklink_ci_probe -SoulseekPassword blacklink_ci_probe
 function WP($p,$s=40){$end=(Get-Date).AddSeconds($s);while((Get-Date)-lt$end){$c=New-Object Net.Sockets.TcpClient;try{$a=$c.BeginConnect('127.0.0.1',$p,$null,$null);if($a.AsyncWaitHandle.WaitOne(500)-and$c.Connected){$c.EndConnect($a);return $true}}catch{}finally{$c.Close()};Start-Sleep -Milliseconds 300};$false}
 foreach($p in 5030,9696,8080,4713){if(-not(WP $p)){& $r -Action status;Get-ChildItem (Join-Path $sm Data) -Recurse -File -ErrorAction SilentlyContinue|Where-Object{$_.Name-match'log|conf|ini|xml|yml'}|ForEach-Object{Write-Host "--- $($_.FullName)";Get-Content $_.FullName -Tail 120 -ErrorAction SilentlyContinue};throw "Port $p failed"}}
...
}finally{try{if(Test-Path (Join-Path $tmp 'smoke\BackendPackV5.ps1')){& (Join-Path $tmp 'smoke\BackendPackV5.ps1') -Action stop}}catch{};Remove-Item $tmp -Recurse -Force -ErrorAction SilentlyContinue}
