...
static const DialogLayout::Align val11 = { 17, DialogLayout::SIDE_BOTTOM, U_DU(6) };
...
static const DialogLayout::Item layoutItems[] =
{
	...
	{ 0, FLAG_HWND, AUTO, AUTO, 0, &hal4, nullptr, &val10 }, // 14. external sources
#ifdef BL_FEATURE_IP_DATABASE
	{ 0, FLAG_HWND, AUTO, AUTO, 0, &hal4, nullptr, &val10 }, // 15. ctrlStoreIP
#else
	{ 0, FLAG_PLACEHOLDER },
#endif
	{ 0, FLAG_HWND, AUTO, AUTO, 0, &hal4, nullptr, &val10 }, // 16. ctrlStoreSettings
	{ 0, FLAG_HWND, AUTO, AUTO, 0, &hal4, nullptr, &val10 }, // 17. ctrlUseGroupTreeSettings
	{ 0, FLAG_HWND, UNSPEC, AUTO, 0, &hal1, &hal2, &val11 }, // 18. "Hubs"
	{ 0, FLAG_HWND, UNSPEC, UNSPEC, 0, &hal1, &hal2, &val6, &val12 } // 19. Hubs ListView
};
...
// SearchFrame IDs identify windows, not individual searches. External Search needs a
// per-search owner ID so a late HTTP callback from a cancelled search can never
// be inserted into the next search in the same window. Search actions run on
// the GUI thread, so this monotonic counter doesn't need synchronization.
static uint64_t externalSearchCounter = 0;

enum
{
	CTRL_INDEX_ONLY_FREE_SLOTS,
	CTRL_INDEX_EXPAND_RESULTS,
	CTRL_INDEX_EXTERNAL_SEARCH,
#ifdef BL_FEATURE_IP_DATABASE
...
SearchFrame::SearchFrame() :
	...
	showOptions(true), onlyFree(false), useExternalSearch(true), droppedResults(0), resultsCount(0),
	expandSR(false),
	storeSettings(false),
...
LRESULT SearchFrame::onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	controls.add(WinUtil::CTRL_CHECKBOX, IDC_OPTION_CHECKBOX, R_(EXTERNAL_SEARCH_SOURCES));
#ifdef BL_FEATURE_IP_DATABASE
	...
	const bool externalAvailable = ExternalSearchManager::getInstance()->hasEnabledBackends();
	controls.setCheck(CTRL_INDEX_EXTERNAL_SEARCH, externalAvailable && useExternalSearch);
	::EnableWindow(controls.getData()[CTRL_INDEX_EXTERNAL_SEARCH].hWnd, externalAvailable);
	...
	layout[13].id = (UINT_PTR) data[CTRL_INDEX_EXTERNAL_SEARCH].hWnd;
#ifdef BL_FEATURE_IP_DATABASE
	layout[14].id = (UINT_PTR) data[CTRL_INDEX_STORE_SEARCH_IP].hWnd;
#endif
	layout[15].id = (UINT_PTR) data[CTRL_INDEX_SAVE_SETTINGS].hWnd;
	layout[16].id = (UINT_PTR) data[CTRL_INDEX_USE_GROUP_TREE].hWnd;
	layout[17].id = (UINT_PTR) data[CTRL_INDEX_LABEL_HUBS].hWnd;
	layout[18].id = (UINT_PTR) ctrlHubs.m_hWnd;
	...
	ExternalSearchManager::getInstance()->addListener(this);
	initHubs();
	treeItemRoot = nullptr;
	treeItemCurrent = nullptr;
	treeItemOld = nullptr;
...
void SearchFrame::onEnter()
{
	...
	if (externalSearchId)
	{
		ExternalSearchManager::getInstance()->cancelSearch(externalSearchId);
		externalSearchId = 0;
	}
	...
	const bool externalAvailableNow = ExternalSearchManager::getInstance()->hasEnabledBackends();
	::EnableWindow(controls.getData()[CTRL_INDEX_EXTERNAL_SEARCH].hWnd, externalAvailableNow);
	controls.setCheck(CTRL_INDEX_EXTERNAL_SEARCH, externalAvailableNow && useExternalSearch);
	const bool externalEnabled = externalAvailableNow && useExternalSearch;
	const int hubCount = ctrlHubs.GetItemCount();
	if (hubCount <= 1 && !externalEnabled)
	{
		setWindowTitle(TSTRING(SEARCH));
	...
	if (searchClients.empty() && !externalEnabled)
	{
		setWindowTitle(TSTRING(SEARCH));
		return;
	}
	...
	
	{
		...
		const unsigned dcSearchTime = searchClients.empty() ? 0 : ClientManager::multiSearch(searchParam, searchClients);
		unsigned externalSearchTime = 0;
		if (externalEnabled && searchParam.fileType != FILE_TYPE_TTH)
		{
			externalSearchId = ++externalSearchCounter;
			externalSearchTime = ExternalSearchManager::getInstance()->search(Text::fromT(searchTarget), externalSearchId);
		}
		searchEndTime = searchStartTime + max(dcSearchTime + max(dhtSearchTime, SEARCH_RESULTS_WAIT_TIME), externalSearchTime);
		updateWaitingTime();
		waitingResults = true;
	}
}
...
void SearchFrame::addExternalSearchResult(const ExternalSearch::Result& result)
{
	if (isClosedOrShutdown() || !externalSearchId || result.ownerId != externalSearchId)
		return;

	// Apply the constraints that have an unambiguous meaning across networks.
	// Unknown external sizes are kept because dropping them hurts rare-file search.
	{
		LOCK(csSearch);
		bool reject = false;
		if (result.size > 0 && searchParam.sizeMode != SIZE_DONTCARE)
		{
			switch (searchParam.sizeMode)
			{
				case SIZE_ATLEAST: reject = result.size < searchParam.size; break;
				case SIZE_ATMOST: reject = result.size > searchParam.size; break;
				case SIZE_EXACT: reject = result.size != searchParam.size; break;
				default: break;
			}
		}
		if (onlyFree && result.network == ExternalSearch::NETWORK_SOULSEEK && !result.freeSlot)
			reject = true;
		if (reject)
		{
			droppedResults++;
	...
	auto searchInfo = new SearchInfo(result);
	{
		LOCK(csEverything);
		everything.insert(searchInfo);
	}
...
void SearchFrame::on(ExternalSearchListener::Result, const ExternalSearch::Result& result) noexcept
{
	if (result.ownerId == externalSearchId) addExternalSearchResult(result);
}

void SearchFrame::on(ExternalSearchListener::Status, uint64_t ownerId, const string& text) noexcept
{
	if (ownerId == externalSearchId) LogManager::message("External Search: " + text, false);
}

void SearchFrame::on(ExternalSearchListener::Error, uint64_t ownerId, const string& text) noexcept
{
	if (ownerId == externalSearchId) LogManager::message(text, false);
}
...
LRESULT SearchFrame::onChangeOption(WORD /*wNotifyCode*/, WORD /*wID*/, HWND hWndCtl, BOOL& /*bHandled*/)
{
	switch (controls.find(hWndCtl))
	{
		...
		case CTRL_INDEX_EXPAND_RESULTS:
			...
			break;
		case CTRL_INDEX_EXTERNAL_SEARCH:
			useExternalSearch = controls.isChecked(CTRL_INDEX_EXTERNAL_SEARCH);
			break;
#ifdef BL_FEATURE_IP_DATABASE
...
int SearchFrame::SearchInfo::compareItems(const SearchInfo* a, const SearchInfo* b, int col, int /*flags*/)
{
	...
	{
		if (col == COLUMN_SIZE || col == COLUMN_EXACT_SIZE)
		{
			const int64_t as = a->external ? a->externalResult.size : a->sr.getSize();
			const int64_t bs = b->external ? b->externalResult.size : b->sr.getSize();
			res = compare(as, bs);
			if (res) return res;
		}
		res = Util::defaultSort(a->getText(col), b->getText(col));
		if (res) return res;
		return Util::defaultSort(a->getText(COLUMN_FILENAME), b->getText(COLUMN_FILENAME));
	}
	switch (col)
	{
...
void SearchFrame::SearchInfo::calcImageIndex()
{
	...
	{
		if (external)
			iconIndex = g_fileImage.getIconIndex(externalResult.name);
		else if (sr.getType() == SearchResult::TYPE_FILE)
...
const tstring& SearchFrame::SearchInfo::getText(uint8_t col) const
{
	...
	if (external)
	{
		switch (col)
		{
			case COLUMN_FILENAME: columns[col] = Text::toT(externalResult.name); break;
			case COLUMN_HITS: break;
			case COLUMN_NICK: columns[col] = Text::toT(externalResult.source); break;
			case COLUMN_TYPE: columns[col] = Text::toT(Util::getFileExtWithoutDot(externalResult.name)); break;
			case COLUMN_SIZE: if (externalResult.size > 0) columns[col] = Util::formatBytesT(externalResult.size); break;
			case COLUMN_EXACT_SIZE: if (externalResult.size > 0) columns[col] = Util::formatExactSizeT(externalResult.size); break;
			case COLUMN_PATH: columns[col] = Text::toT(externalResult.path); break;
			case COLUMN_SLOTS:
				if (externalResult.network == ExternalSearch::NETWORK_SOULSEEK)
				{
					columns[col] = externalResult.freeSlot ? _T("+") : _T("-");
					if (externalResult.queueLength > 0) columns[col] += _T(" / Q:") + Util::toStringT(externalResult.queueLength);
				}
				else if (externalResult.network == ExternalSearch::NETWORK_BITTORRENT)
					columns[col] = _T("S:") + Util::toStringT(externalResult.seeders) + _T(" L:") + Util::toStringT(externalResult.leechers);
				else if (externalResult.network == ExternalSearch::NETWORK_ED2K)
				{
					columns[col] = _T("S:") + Util::toStringT(externalResult.sourceCount);
					if (externalResult.completeSourceCount > 0) columns[col] += _T(" C:") + Util::toStringT(externalResult.completeSourceCount);
				}
				break;
			case COLUMN_HUB:
				columns[col] = Text::toT(externalResult.networkName);
				if (!externalResult.backendName.empty()) columns[col] += _T(" / ") + Text::toT(externalResult.backendName);
				break;
			case COLUMN_TTH:
				if (!externalResult.hash.empty()) columns[col] = Text::toT(externalResult.hashType.empty() ? externalResult.hash : externalResult.hashType + ":" + externalResult.hash);
				break;
			default: break;
		}
		colMask |= 1<<col;
...
void SearchFrame::downloadExternal(SearchInfo* si)
{
	if (!si || !si->external) return;
	if (ExternalSearchManager::getInstance()->enqueueDownload(si->externalResult))
		return;

	// If a BitTorrent download backend is disabled, retain the Stage 1 fallback
	// and open the magnet/.torrent URL in the system handler.
	if (si->externalResult.network == ExternalSearch::NETWORK_BITTORRENT && !si->externalResult.downloadUri.empty())
	{
		WinUtil::openLink(Text::toT(si->externalResult.downloadUri));
		return;
	}
	LogManager::message("External Search: для результата не настроен доступный backend скачивания", false);
}
...
LRESULT SearchFrame::onDoubleClickResults(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
{
	...
	if (item->iItem != -1)
	{
		...
		if (ctrlResults.GetSelectedCount() == 1)
		{
			...
			if (si && !si->external && isValidFile(si->sr))
			{
				string realPath;
...
LRESULT SearchFrame::onClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL & bHandled)
{
	...
	if (!closed)
	{
		...
		ExternalSearchManager::getInstance()->removeListener(this);
		if (externalSearchId) ExternalSearchManager::getInstance()->cancelSearch(externalSearchId);
		externalSearchId = 0;
		framesLock.lock();
...
void SearchFrame::runUserCommand(UserCommand & uc)
{
	...
	{
		SearchInfo* info = ctrlResults.getItemData(sel);
		if (!info || info->external) continue;
		const SearchResult& sr = info->sr;
...
LRESULT SearchFrame::onSearchByTTH(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	while ((i = ctrlResults.GetNextItem(i, LVNI_SELECTED)) != -1)
	{
		SearchInfo* info = ctrlResults.getItemData(i);
		if (!info || info->external) continue;
		const SearchResult sr = info->sr;
		if (sr.getType() == SearchResult::TYPE_FILE)
		{
...
void SearchFrame::addSearchResult(SearchInfo* si)
{
	...
	if (!si->external)
	{
		...
		if (ip.type && user) user->setIP(ip);
	}
	// Check previous DC search results for dupes. External backends have their own deduplication.
	const auto& parents = ctrlResults.getParents();
	if (!si->external && user && !sr.getTTH().isZero())
	{
	...
	if (running)
	{
		...
		if (si->external || sr.getType() == SearchResult::TYPE_FILE || sr.getType() == SearchResult::TYPE_DIRECTORY)
		{
			const string& file = si->external ? si->externalResult.name : sr.getFileName();
			int fileType = (!si->external && sr.getType() == SearchResult::TYPE_DIRECTORY) ? FILE_TYPE_DIRECTORY : getPrimaryFileType(file, true);
		...
		}
		{
			...
			{
				if (!si->external && !si->getText(COLUMN_TTH).empty())
...
int SearchFrame::makeTargetMenu(const SearchInfo* si, OMenu& menu, int idc, ResourceManager::Strings title)
{
	...
	if (si && !si->external && si->sr.getType() == SearchResult::TYPE_FILE && idc != IDC_DOWNLOADDIR_TO_FAV)
	{
	...
	if (reinterpret_cast<HWND>(wParam) == ctrlResults && ctrlResults.GetSelectedCount() > 0)
	{
		...
		if (selCount)
		{
			...
			if (selCount == 1)
			{
				...
				if (si && !si->external) sr = &si->sr;
			}

			bool hasExternal = false;
			for (int i = ctrlResults.GetNextItem(-1, LVNI_SELECTED); i != -1; i = ctrlResults.GetNextItem(i, LVNI_SELECTED))
			{
				SearchInfo* item = ctrlResults.getItemData(i);
				if (item && item->external) { hasExternal = true; break; }
			}
			if (hasExternal)
			{
				OMenu externalMenu;
				externalMenu.CreatePopupMenu();
				dlTargets.clear();
				dlTargets[IDC_DOWNLOAD_TO_FAV] = DownloadTarget();
				externalMenu.AppendMenu(MF_STRING, IDC_DOWNLOAD_TO_FAV, CTSTRING(DOWNLOAD), g_iconBitmaps.getBitmap(IconBitmaps::DOWNLOAD, 0));
				externalMenu.AppendMenu(MF_SEPARATOR);
				externalMenu.AppendMenu(MF_STRING, IDC_COPY_FILENAME, CTSTRING(FILENAME));
				externalMenu.AppendMenu(MF_STRING, IDC_COPY_LINK, CTSTRING(COPY_LINK));
				externalMenu.AppendMenu(MF_STRING, IDC_COPY_SIZE, CTSTRING(SIZE));
				externalMenu.AppendMenu(MF_SEPARATOR);
				externalMenu.AppendMenu(MF_STRING, IDC_REMOVE, CTSTRING(REMOVE), g_iconBitmaps.getBitmap(IconBitmaps::REMOVE, 0));
				externalMenu.SetMenuDefaultItem(IDC_DOWNLOAD_TO_FAV);
				externalMenu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, m_hWnd);
				return TRUE;
			}
...
LRESULT SearchFrame::onCopy(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	while ((i = ctrlResults.GetNextItem(i, LVNI_SELECTED)) != -1)
	{
		...
		if (si->external)
		{
			switch (wID)
			{
				case IDC_COPY_NICK: sCopy = si->externalResult.source; break;
				case IDC_COPY_FILENAME: sCopy = si->externalResult.name; break;
				case IDC_COPY_PATH: sCopy = si->externalResult.path; break;
				case IDC_COPY_SIZE: sCopy = Util::formatBytes(si->externalResult.size); break;
				case IDC_COPY_HUB_URL: sCopy = si->externalResult.networkName + " / " + si->externalResult.backendName; break;
				case IDC_COPY_LINK:
				case IDC_COPY_FULL_MAGNET_LINK:
				case IDC_COPY_WMLINK: sCopy = si->externalResult.downloadUri; break;
				case IDC_COPY_TTH: sCopy = si->externalResult.hash; break;
				default: break;
			}
		}
		else
...
LRESULT SearchFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/)
{
	...
	switch (cd->nmcd.dwDrawStage)
	{
		...
		case CDDS_SUBITEM | CDDS_ITEMPREPAINT:
		{
			...
			// External Search rows don't have a DC User/IP/TTH object. Render their
			// already-normalized text directly and skip DC-only decorators.
			if (si->external)
			{
				if (cd->iSubItem == 0)
...
bool SearchFrame::matchFilter(const SearchInfo* si, int sel, bool doSizeCompare, FilterModes mode, int64_t size)
{
	...
	if (doSizeCompare)
	{
		const int64_t itemSize = si->external ? si->externalResult.size : si->sr.getSize();
		bool insert = true;
		switch (mode)
		{
			case EQUAL: insert = (size == itemSize); break;
			case GREATER_EQUAL: insert = (size <= itemSize); break;
			case LESS_EQUAL: insert = (size >= itemSize); break;
			case GREATER: insert = (size < itemSize); break;
			case LESS: insert = (size > itemSize); break;
			case NOT_EQUAL: insert = (size != itemSize); break;
		}
		return insert;
	}
...
void SearchFrame::insertStoredResults(HTREEITEM treeItem, int filter, bool doSizeCompare, FilterModes mode, int64_t size)
{
	...
	for (SearchInfo* si : data)
		if (matchFilter(si, filter, doSizeCompare, mode, size))
		{
			...
			si->hits = -1;
			si->stateFlags = 0;
			if (!si->external && si->sr.getType() == SearchResult::TYPE_FILE && !si->sr.getTTH().isZero())
				ctrlResults.insertGroupedItem(si, expandSR, true);
			else
				ctrlResults.insertItem(si, I_IMAGECALLBACK);
		}
...
void SearchFrame::getSelectedUsers(vector<UserPtr>& v) const
{
	...
	{
		SearchInfo* si = ctrlResults.getItemData(i);
		if (si && si->getUser()) v.push_back(si->getUser());
